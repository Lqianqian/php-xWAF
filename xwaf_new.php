...
class xWAF {
	...
	function getArray($Type) { //规则尚不完善 、 实际项目使用建议使用nginx+lua
		switch ($Type) {
			case 'SQL':
				// 字符串为 不区分大小写 进行搜索
				// 数组为 不区分大小写 进行正则匹配
				// ['SELECT (.*)FROM'] === ['SELECT ','FROM']  最终效果一致
				// [[:blank:]] 代表空格符个制表符
				// . 匹配除换行符 \n 之外的任何单个字符
				// .+ 匹配除换行符 \n 之外的任何多个字符
				return array(
	...
	function sqlCheck($Value, $Method, $DisplayName) {
		...
		foreach ($BadWords as $BadWord) {
			if (is_string($BadWord) && strpos(strtolower($Value), strtolower($BadWord)) !== false) {
				// String contains some Vuln.
				$this->vulnDetectedHTML($Method, $BadWord, $Value, 'SQL Injection');
			}else if(is_array($BadWord)){
				preg_match_all('/'.implode('(.*)', $BadWord).'/iUs', $Value, $matches);
				if(!empty($matches[1])){
					$this->vulnDetectedHTML($Method, implode('(.*)', $BadWord), $Value, 'SQL Injection');
				}
			}
		}
	}
	...
	function santizeString($String) {
		...
		foreach ($XSS as $replace) {
			if (is_string($replace)) {
				...
				preg_match_all('/'.implode('(.*)', $replace).'/iUs', $String, $matches);
				if(!empty($matches[1])){
